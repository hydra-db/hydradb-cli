...
- **`HYDRADB_ACL` defaults `--acl`.** `query`, `list`, `inspect`, `relations`, `subgraph` and the deprecated aliases read comma- or whitespace-separated principals from the environment when `--acl` is omitted. `--acl` on the command line wins. Omitting both searches everything the API key can reach — an empty list is never sent, because the API treats `[]` as unrestricted, not as "nobody". Not applied to `graph query` (BYOG is a different store).
